...
每位審查者完成審查後，必須立即將結果與具體意見寫入設計文件 `## 審查` 段落中對應的 Reviewer 區塊（見「文件回寫規則」章節）。未回寫至文件的審查視為未完成。
...
任務追蹤於 session SQL 資料庫（`todos` 資料表）以及設計文件的 `## 任務` 段落中（詳見「文件回寫規則」章節）。
...
程式碼審查完成後，必須立即將審查結果、發現問題與修正記錄寫入設計文件 `## 程式碼審查` 段落（見「文件回寫規則」章節）。未回寫至文件的審查視為未完成。
...
> **所有審查意見與狀態變更，都必須即時回寫至設計文件本身。設計文件是唯一的真實來源。**

本專案不依賴外部工具（如 issue tracker、PR comment）記錄審查結果或功能狀態。
所有記錄都必須存在於設計文件中，確保任何人僅憑閱讀設計文件即可掌握完整脈絡。

### 狀態回寫

每次功能狀態發生變更時，必須**立即**更新設計文件 `## 狀態` 欄位。不得批次回補。

| 觸發事件 | 新狀態 | 必須同步更新 |
|---------|--------|-------------|
| 設計文件完成，準備送審 | `design_complete` | `## 狀態` |
| 送交審查者 | `in_review` | `## 狀態` |
| 任一審查者回覆 REVISE | `revise_requested` | `## 狀態` + `## 審查` 中該審查者的意見 |
| 兩位審查者都回覆 APPROVED | `approved` | `## 狀態` + `## 審查` 中兩位審查者的意見 |
| 任務清單產生完成 | `tasks_generated` | `## 狀態` + `## 任務` |
| 開始實作第一個任務 | `in_progress` | `## 狀態` |
| 所有任務完成，送交程式碼審查 | `code_review` | `## 狀態` |
| 程式碼審查通過 | `done` | `## 狀態` + `## 程式碼審查` |

### 審查意見回寫

**設計審查（第二階段）：**

每位審查者完成審查後，必須將以下內容寫入設計文件 `## 審查` 段落對應的 Reviewer 區塊：

1. **狀態**：`APPROVED`、`REVISE` 或 `REJECTED`
2. **具體意見**：審查中發現的所有問題、建議或肯定的要點

若審查者回覆 REVISE，設計者處理後須在同一區塊追加修正說明，審查者重新審查後再次更新狀態與意見。
歷次審查的意見**保留不刪除**，以完整記錄審查歷程。

**程式碼審查（第五階段）：**

程式碼審查完成後，必須將以下內容寫入設計文件 `## 程式碼審查` 段落：

1. **審查結果**：`PASS` 或 `FIX_REQUIRED`
2. **發現問題**：審查中發現的所有問題（若為 PASS 且無問題，填「無」）
3. **修正記錄**：針對每個問題的修正方式與對應 commit（若有 FIX_REQUIRED 循環）

### 任務狀態回寫

每個任務狀態變更時，必須同步更新以下**兩處**：

1. **SQL `todos` 資料表** — 更新 `status` 欄位
2. **設計文件 `## 任務` 段落** — 完成的任務標記為「✅ 已完成」
...
- **設計文件是唯一的真實來源。** 所有審查意見、狀態變更、任務進度都必須回寫至設計文件本身。未記錄於文件中的審查或狀態變更視為不存在。
- 任何實作某功能的 PR 或 commit，在合併前都必須有對應的設計文件，且其狀態為 `approved` 或更後期的狀態。
- `docs/designs/` 目錄是所有已設計並已核准功能的權威記錄。
- 審查者必須將審查意見直接記錄在設計文件的 `## 審查` 段落中。審查結果未回寫至文件的，視為審查未完成。
- 程式碼審查結果必須記錄在設計文件的 `## 程式碼審查` 段落中。未記錄的程式碼審查不構成通過依據。
